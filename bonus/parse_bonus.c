...
/*   Updated: 2022/02/09 03:44:59 by hakim            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	split_n_put(char *line, t_data *data, int row)
{
	...
	while (++index < data->width)
	{
		...
		if (data->map[row][index] < data->z_min)
			data->z_min = data->map[row][index];
		if (data->map[row][index] > data->z_max)
			data->z_max = data->map[row][index];
	}
